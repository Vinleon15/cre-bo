...
async def cmd_update(msg: types.Message):
    ...
    # Два разбора разом — это двойная нагрузка на модель и лишние отказы
    # по лимиту частоты. Второй заход просто отклоняем.
    if extractor.is_busy():
        return await msg.answer(
            "Разбор уже идёт — дождитесь ответа, повторно запускать не нужно."
        )
